...
def print_all_to_file(file_name, *args):
...
def refresh_screen(scr, matrix, animated, delay=STANDARD_DELAY):
    ...
    """
    if not animated or scr is None:
        return
...
def show_path(scr, path, animated, delay=PATH_DELAY):
    ...
    """
    if not animated or len(path) == 0 or scr is None:
        return
...
def show_final_message(scr, x, y, animated, message="Press any button to exit..."):
